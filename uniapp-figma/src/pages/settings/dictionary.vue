    ...
    <view v-if="editorOpen" class="sheet-mask" @click="closeEditor">
      <view class="editor-sheet" @click.stop>
        ...
        <view v-if="type !== 'brands'" class="field">
          <text>上级</text>
          <picker :range="parentOptions" range-key="pathLabel" @change="pickParent">
            <view class="picker">{{ parentLabel }}</view>
          </picker>
        </view>
...
const configs = {
  ...
  parentOptions = computed(() => [
    { id: 0, name: "无上级", pathLabel: "无上级", depth: 0 },
    ...items.value.filter((item) => !forbiddenParentIds.value.has(item.id)),
  ]),
  parentLabel = computed(
    () =>
      parentOptions.value.find((item) => item.id === form.parentId)?.pathLabel ||
      "无上级",
  );
...
function pickParent(event: any) {
  const selected = parentOptions.value[Number(event.detail.value)];
