...
export const api = {
  ...
  updateWishlistPrice: (id: number, currentPrice: number) =>
    apiRequest<void>(`/wishlist/${id}/price`, {
      method: "PATCH",
      data: { currentPrice, capturedAt: new Date().toISOString() },
    }),
  markPurchased: (id: number, p: AssetPayload) =>
    apiRequest<number>(`/wishlist/${id}/mark-purchased`, {
      method: "POST",
      data: p,
    }),
