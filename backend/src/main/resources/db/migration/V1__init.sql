-- DigiLedger schema initialization via Flyway（当前完整结构基线）
...
CREATE TABLE IF NOT EXISTS wishlist (
  ...
  source VARCHAR(200) COMMENT '心愿来源/种草途径',
  notes TEXT COMMENT '备注',
  priority TINYINT NOT NULL DEFAULT 3 COMMENT '优先级',
  converted_asset_id BIGINT COMMENT '已转化资产ID',
  purchased_at DATE COMMENT '确认购买日期',
  purchased_price DECIMAL(12,2) COMMENT '确认购买价格',
  purchase_price_diff DECIMAL(12,2) COMMENT '购买价减目标价',
  created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  updated_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
