...
public class AssetCoverService {
    ...
    public CoverApplyResponse setCoverFromUrl(Long assetId, @NotNull String sourceUrl) {
        ...
        String extra = toExtra(Map.of("type", "cover-url", "sourceUrl", sourceUrl));
        AttachmentResponse attachment = attachmentService.upload(file, "ASSET_COVER", assetId, extra);
    ...
    public CoverApplyResponse removeBackground(Long assetId, String objectKey) {
        ...
        String extra = toExtra(Map.of("type", "remove-bg", "objectKey", objectKey));
        AttachmentResponse attachment = attachmentService.upload(
                new InMemoryMultipartFile("file", "no-bg.png", "image/png", result),
                "ASSET_COVER",
                assetId,
                extra
        );
    ...
    public String toExtra(Map<String, Object> payload) {
        try {
            ...
            throw new BizException(ErrorCode.INTERNAL_ERROR, "构建附件元信息失败");
        }
    }
