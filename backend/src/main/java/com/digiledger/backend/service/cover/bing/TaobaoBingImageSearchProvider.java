...
/**
 * 淘宝
 * 使用 Bing 图片搜索的淘宝站内结果作为最高优先级的封面候选来源。
 */
@Component
@Order(5)
public class TaobaoBingImageSearchProvider implements ImageSearchProvider {

    private static final Logger log = LoggerFactory.getLogger(TaobaoBingImageSearchProvider.class);
    private static final String SOURCE = "BING_TAOBAO_IMAGE_SEARCH";
    ...
    public TaobaoBingImageSearchProvider(RestTemplateBuilder builder,
    ...
    public List<CoverCandidate> search(String query, int limit) {
        ...
        log.debug("Bing 淘宝图源查询编码：{}", encodedQuery);
        String queryWithSite = encodedQuery + "+site:taobao.com";
        URI uri = UriComponentsBuilder.fromHttpUrl(ENDPOINT)
                ...
                .build(true)
                .toUri();
        log.debug("Bing 淘宝图源请求：{}", uri);
        try {
            String body = restTemplate.getForObject(uri, String.class);
            return parseHtml(body, target);
        } catch (RestClientException ex) {
            log.warn("无法查询 Bing 淘宝图源：{}", ex.getMessage());
            return List.of();
        }
    }
    ...
    private List<CoverCandidate> parseHtml(String body, int limit) {
        ...
        for (Element item : items) {
            ...
            try {
                ...
                log.debug("解析 Bing 淘宝图源结果失败：{}", ex.getMessage());
            }
        }
        return candidates;
    }
