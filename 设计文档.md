...
CREATE TABLE device_asset (
  ...
  wish_status ENUM('VERY_SATISFIED','SATISFIED','NEUTRAL','WANT_TO_SELL','MUST_SELL') DEFAULT 'SATISFIED',
  purchase_id BIGINT,                 -- 主购买ID（冗余）
  purchase_date DATE,                 -- 主购买日期（冗余）
  purchase_platform VARCHAR(120),
  enabled_date DATE,                  -- 启用日期（默认=主购买日期）
  retired_date DATE,
  warranty_channel VARCHAR(120),
  warranty_expire_date DATE,
  related_assets JSON,                -- 心愿/配件关联
  tags JSON,
  cover_image_url VARCHAR(500),
  purchase_proof_url VARCHAR(500),
  notes TEXT,
  created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  updated_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
...
CREATE TABLE maintenance_record (
  ...
  created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  updated_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  INDEX idx_asset_date (asset_id, maintenance_date),
  CONSTRAINT fk_maintenance_asset FOREIGN KEY (asset_id) REFERENCES device_asset(id)
);
...
> * `device_asset` 增加 `wish_status`、`purchase_platform`、`warranty_*`、`related_assets`、`purchase_proof_url` 等字段，以支撑心愿管理、保修提醒与凭证展示。
> * `device_asset.purchase_id`/`purchase_date` 为冗余字段，便于列表查询与排序。
> * `maintenance_record` 记录维修/保养/配件维护历史，供前端资产维护页展示。
> * `purchase.item_name` 保留配件/服务名称，配合 `type` 区分主购买与附加项目。
...
* `POST /api/assets`
  ...
  * 创建资产（含主购买），请求：
  ...
  ```json
  {
    ...
    "purchase_platform":"JD",
    "enabled_date":"2024-09-15",
    "warranty_channel":"Apple Care+",
    "warranty_expire_date":"2026-09-14",
    "related_asset_ids":[1002,1003],
    "purchase_proof_url":"https://example.com/invoice.pdf",
    "tags":["自用","摄影"],
    "notes":"",
    "purchase":{
      "item_name":"iPhone 14 Pro 256G",
      "type":"PRIMARY",
      "platform":"JD",
      "seller":"Apple京东自营",
      "price":8999.00,
...
* `GET /api/assets`
  ...
  * 查询参数：`status? wish_status? category? brand? platform? tag? q? year? warranty_expire_in? page? size? sort?`（sort 示例：`purchase_date:desc`）
...
* `GET /api/assets/{id}/maintenance` → 查询资产维保记录，支持 `?page&size&from&to&type`。
* `POST /api/maintenance` → 新增维保，body：`{ asset_id, title, maintenance_type?, maintenance_date, cost, provider?, notes?, attachments? }`。
* `PUT /api/maintenance/{id}` → 修改维保记录。
* `DELETE /api/maintenance/{id}` → 删除维保记录（留操作日志，禁止批量硬删）。

### 5.7 提醒 / 仪表盘小提醒

* `GET /api/reminders/warranty?days=30` → 返回未来 `days` 天内即将到期的资产保修：`[{ asset_id, name, warranty_expire_date, days_left }]`。
* `GET /api/reminders/maintenance?inactive_days=180` → 返回超过指定天数未维护的资产，用于“需要保养”提醒。
* `GET /api/metrics/overview` 中补充 `depreciation_total`、`avg_cost_total` 等 KPI，供仪表盘概览使用。
...
* **仪表盘（Dashboard）**：包含概览 KPI、资产列表（支持新增/编辑/删除）、资产维护信息、购置记录列表与小提醒时间线，行点击同步右侧详情，弹窗表单支持动态添加配件/维保记录。
* **资产详情页**：按卡片展示基本信息、财务指标、购置信息、维保与售出记录，并在“相关设备”区域展示 `related_assets`，支持快速跳转。
* **资产表单页**：拆分步骤录入基本资料、折旧参数、主购买与附属配件、保修信息与购买凭证，实时预览折旧计算结果。
* **售出向导**：强调盈亏即时计算、平台费用拆分、售后提醒（生成提醒任务或邮件通知）。
* **提醒中心/小组件**：按接口返回类型（保修、维护、待售）以时间线或卡片展示，并支持点击跳转资产详情。
