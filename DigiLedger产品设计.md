...
- **封面智能增强**：新增智能找图入口（新建/编辑、列表卡片未设置封面）与后台远程下载能力，并提供已有封面一键抠图（remove.bg），所有封面统一通过附件/对象存储，前端只读取完整 URL 展示。
...
- **使用统计**：按天均摊；所有时长/成本计算均以购买日期为起点，涵盖当前/售出/报废等状态。
...
> 物品创建/编辑使用：`category_id`（父或叶）、`platform_id`、`brand_id`、`tag_ids[]`。查询支持 `category_id`（含子级）、`platform_id`、`tag_ids[]` 过滤；**列表字段“购买时间”为统一时间起点。**
...
- **V0.4.0（升级路线重构）**
  - 升级路线支持实际履历、规划和混合三种类型，以及进行中、已完成、已归档状态。
  - 路线通过首节点原子创建；可从任意物品添加上级、同级或下级，图谱展示一、二、三代和年月信息。
  - 节点区分真实物品与计划物品。真实物品展示图片、购买时间、购买金额、使用天数和出售情况；计划物品独立记录周期和预算。
  - 前后代关系展示购买时间相差天数、购买价差；上级主商品已经出售时，额外展示下级购买价格与出售净收入的实际补款。
  - 路线汇总分离真实支出、出售收入、净投入与计划预算，避免计划金额混入实际账目。
  - 数据库新增路线类型/状态、节点类型/同级用途、关系类型字段；由 Flyway `V3__rebuild_upgrade_route.sql` 迁移。
  - V1.1 改为心愿单驱动的未来节点：新增 `equip_upgrade_route.main_asset_id`、`equip_upgrade_node.wishlist_id` 和 `is_mainline`。心愿购买后自动替换路线节点；同级备用物品只参与路线总收支，不参与上下级差价。由 Flyway `V6__upgrade_route_wishlist_and_mainline.sql` 迁移。
...
- **升级路线**：由线性年度计划改为可分支的物品升级图，创建时同时选择第一个物品或计划物品。
- **关系与计算**：支持上级、同级、下级；显示购买时间间隔、购买价差和主商品出售后的实际升级补款。
- **展示**：路线图显示图片、名称、购买年月、代际、金额、使用时间与出售状态；全年预算与真实收支分开汇总。
- **数据约束**：禁止重复物品、自环、跨路线关系和成环；归档路线只读。
...
- **新建物品**：下拉新建类别/标签/平台/品牌；品牌字典；删除报废日期与冗余字段，统一以购买日期作为时间起点；配件/服务新增名称；日期支持“今天”。
