...
export interface AssetQuery { status?: string; keyword?: string; categoryId?: number; platformId?: number; tagIds?: number[] }
...
export const fetchAssets = (filters: AssetQuery = {}) => http.get<AssetSummary[]>('/assets', { params: { status: filters.status || undefined, q: filters.keyword || undefined, category_id: filters.categoryId, platform_id: filters.platformId, tag_ids: filters.tagIds?.join(',') } })
export const fetchAssetsPage = (filters: AssetPageQuery = {}) => http.get<AssetPage>('/assets/page', { params: { status: filters.status || undefined, q: filters.keyword || undefined, category_id: filters.categoryId, platform_id: filters.platformId, tag_ids: filters.tagIds?.join(','), page: filters.page || 1, page_size: filters.pageSize || 20, sort_by: filters.sortBy || 'purchaseDate', sort_order: filters.sortOrder || 'desc' } })
